...
def get_images_from_query(query):
    ...
    image_elements = soup.select("td>div>div>div>div>table", limit=10)
    print(len(image_elements))
    with open("./test.html", "w") as f:
        f.write(str(soup))
    return_queue = []
    for i in range(len(image_elements)):
        image_url = image_elements[i].select_one("img").get("src")
        image_title = image_elements[i].select_one("td>a>div>span>span").text
...
def get_related_images(image_title):
    ...
    image_elements = soup.select("td>div>div>div>div>table", limit=5)
    # print(len(image_elements))
    with open("./test.html", "w") as f:
        f.write(str(soup))
    return_queue = []
...
def get_image_tags(urls, titles):
    print(urls, titles)
    tags = []
    for url in urls:
        ...
        for x in id_tags.split(","):
            tags.append(x)
    for x in range(3):
        tags.append(random.choice(titles))
    return tags
